...
class Module:
    ...
    def _call_impl(self, *input, **kwargs):
        ...
        if type(input) is tuple:
            input_clone = input[0].clone()
        else:
            input_clone = input.clone()
        result = forward_call(*input, **kwargs)
        input = input_clone # TODO(wby) here we made a clone process in order to avoid input changed in forward_call function, but it may cause some problems
